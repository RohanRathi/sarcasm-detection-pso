...
class particle():
    ...
    def __init(self, init_pos):
        self.w = 0.5            #Init weight as .5
        self.pos = []
        self.vel = []
        self.pbest = []
        self.min_err = 1
    ...
    def update_vel(self):
        ...
        self.w = self.w + .5 * math.exp(-sqrt(x))

        for i in range(0, num_dimensions):
            r1 = random.random()        #TODO: c1, c2, r1, r2
            r2 = random.random()

            vel_cog = 2 * r1 * (pbest[i] - pos[i])
            vel_soc = 2 * r2 * (gbest[i] - pos[i])
            self.vel[i] = w * self.vel[i] + vel_cog + vel_soc
...
class pso():
    ...
    def __init__(self):
        ...
        for i in range(0, num_particles):
            swarm.append(self.init_particle_pos())
    ...
    def init_particle_pos(self):
        ...
        for i in range(0, numdimensions):
            position.append((rand() % (max_bound[i] - min_bound[i]) / inc[i]) * inc[i] + min_bound[i])
        return position
...
class train_model(sarcasm_model):
    ...
    def __init__(self, train_file, validation_file, word_file_path, split_word_path, emoji_file_path, model_file,
        ...
        # training
        model.fit(X, Y, batch_size=512, epochs=5, validation_data=(tX, tY), shuffle=True,
