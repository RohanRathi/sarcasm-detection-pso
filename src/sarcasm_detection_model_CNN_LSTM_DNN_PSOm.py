...
class train_model(sarcasm_model):
    ...
    def build_tmp_model(self, swarm, final):
        ...
        if final != True:
            ...
            # training
            hist = model.fit(self.X, self.Y, batch_size=64, epochs=10, validation_data=(self.tX, self.tY), shuffle=True,
...
class particle():
    ...
    def update_vel(self, gbest, inc):
        ...
        self.w = 0.5 * self.w + .5 * math.exp(-math.sqrt(x))
...
def process_triple(item):
    return item.model.build_tmp_model(item.particle.pos, False)
...
mparticle = collections.namedtuple('mparticle', [
    'particle',
    'model',
])
...
class pso(train_model):
    ...
    def __init__(self):
        ...
        for i in range(0, self.maxiter):
            for j in range(0, self.num_particles, 2):      

                pool = multiprocessing.Pool(initializer=init)
                mparticles = (
                    mparticle(particle=swarm[j], model=tr),
                    mparticle(particle=swarm[j + 1], model=tr),
                )
                pool_err = pool.map(process_triple, mparticles)
                pool.close()	
                pool.join()	

                #swarm[j].update_min_error(tr.build_tmp_model(swarm[j].pos, False))

                for k in range(0, len(pool_err)):
                    swarm[j + k].update_min_error(pool_err[k])
                    if pool_err[k] < self.gbest_err:
                        self.gbest = swarm[j + k].pos
                        self.gbest_err = swarm[j + k].err
...
if __name__ == "__main__":
    ...
    t.load_trained_model(weight_file='weights.10__.hdf5')
    t.predict(test_file)
